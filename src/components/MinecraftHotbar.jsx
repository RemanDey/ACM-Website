...
const MinecraftHotbar = () => {
  ...
  const navItems = [
    { name: "Team", icon: "💎" }, // Diamond Pickaxe substitute
    { name: "Events", icon: "🪓" },
    { name: "About", icon: "⚔️" },
    { name: "Contact", icon: "🌿" },
    { name: "", icon: "" },
  ...
  useEffect(() => {
    const handleKeyPress = (e) => {
      ...
      if (key >= 1 && key <= 9) setActiveIndex(key - 1);
    };
  ...
  return (
    <nav className="fixed bottom-4 left-1/2 -translate-x-1/2 select-none flex flex-col items-center z-100">
      ...
      {/* HOVER LABEL (Appears above everything when hovering) */}
      <div className="h-6 mb-2"> 
        {hoveredIndex !== null && navItems[hoveredIndex].name && (
          <div 
            className="text-white text-sm transition-opacity duration-150"
      ...
      <div className="relative w-[364px] mb-1">
         ...
         <span className="absolute -top-5 left-1/2 -translate-x-1/2 text-[#7cfc00] text-lg font-bold tracking-tight" 
      ...
      <div className="relative flex items-center bg-[#00000066] border-[4px] border-black p-[1px]">
        ...
        {navItems.map((item, index) => (
          <div
            ...
            onMouseEnter={() => setHoveredIndex(index)} // Trigger hover
